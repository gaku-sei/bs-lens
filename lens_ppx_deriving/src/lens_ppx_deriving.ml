...
type lens_options = {prefix: bool; submodule: bool}

let lens_default_options = {prefix= false; submodule= false}
...
let bool_option deriver name expr =
  ...
  | _ ->
      raise_errorf ~loc:expr.pexp_loc
        "%s %s option must be either true or false" deriver name

let parse_options options =
  options
  |> List.fold_left
       (fun deriver_options (name, expr) ->
         match name with
         | "prefix" | "affix" ->
             {deriver_options with prefix= bool_option deriver name expr}
         | "submodule" ->
             {deriver_options with submodule= bool_option deriver name expr}
         | _ ->
             raise_errorf ~loc:expr.pexp_loc "%s does not support option %s"
               deriver name )
...
let define_module loc module_name expressions =
  let expressions =
    List.map
      (fun x -> {pstr_desc= Pstr_value (Nonrecursive, [x]); pstr_loc= loc})
      expressions
  in
  Pstr_module
...
let lens_name ~deriver_options record_type_decl field_name =
  if deriver_options.submodule then field_name
  else if deriver_options.prefix then
    Ppx_deriving.mangle_type_decl
      (`PrefixSuffix (deriver, field_name))
      record_type_decl
...
let module_name ~deriver_options {ptype_name= {txt= name; _}; _} =
  if deriver_options.prefix then String.capitalize_ascii name ^ "Lens"
  else "Lens"

let wrap_in_submodule_sig ~deriver_options record loc signatures =
  if deriver_options.submodule then
      ...
      |> List.map (fun {pld_name= {txt= name; _}; _} ->
             ...
             , [%expr
                 Lens.
                   { get=
                       (fun r ->
                         [%e Exp.field (evar "r") (mknoloc (Lident name))] )
...
let type_named name = Typ.mk (Ptyp_constr (mknoloc (Lident name), []))

let sig_of_type ~options ~path:_
    ({ptype_loc= loc; ptype_name= {txt= record_name; _}; _} as type_decl) =
  let deriver_options = parse_options options in
  match type_decl.ptype_kind with
  | Ptype_record labels ->
      labels
      |> List.map (fun {pld_name= {txt= name; _}; pld_type; _} ->
             let lens_type =
               [%type: ([%t type_named record_name], [%t pld_type]) Lens.t]
             in
             Sig.value
               (Val.mk
                  (mknoloc (lens_name ~deriver_options type_decl name))
                  lens_type) )
